...
# Marker embedded in every file we generate. When the user customizes
# displays via GNOME Settings, mutter rewrites monitors.xml from scratch and
# the marker disappears -- from then on we never touch that user's file.
# Files still carrying the marker are ours and get refreshed when the
# attached monitor changes (stale specs make mutter fall back to LVDS).
MARKER = "<!-- generated by gen-monitors; manual edits via GNOME Settings are preserved -->"


def mode_rate(w):
    # ponytail: LT9611UXD's 4-lane MIPI DSI caps 4k at 30Hz -- the kernel
    # prunes 3840x2160@60 from the mode list, and mutter rejects a stored
    # config whose rate isn't in that list ("Invalid mode ... (60.000)").
    # Board-specific ceiling; revisit if a different HDMI bridge is fitted.
    return "30.000" if w >= 3840 else "60.000"
...
def build_xml(displays):
    ...
    if others:
        ...
        lines.append("    </disabled>")
    lines += ["  </configuration>", "</monitors>", MARKER, ""]
    return "\n".join(lines)
...
def main():
    ...
    # 2) /etc/skel for any future useradd. Ours to manage (marker check for
    #    robustness in case an image customization dropped a file there).
    try:
        skel_ours = MARKER in SKEL_PATH.read_text()
    except (FileNotFoundError, OSError):
        skel_ours = True
    if skel_ours:
        write_atomic(SKEL_PATH, xml, mode=0o644)

    # 3) Existing user homes: refresh monitors.xml while it is still the one
    #    we generated (marker present). A user-customized file (rewritten by
    #    mutter via GNOME Settings, marker gone) is never touched.
    if HOMES_DIR.is_dir():
        for home in sorted(HOMES_DIR.iterdir()):
            ...
            if existing is not None and MARKER not in existing:
                continue  # user-customized: hands off
            if existing == xml:
